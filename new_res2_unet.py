...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        # B, C/4, H, W -> B, C, H, W
        self.conv3 = nn.Conv2d(
            in_channels // 4, n_filters, kernel_size, padding=conv_padding, bias=False
        )
...
class Res2_UNet(nn.Module):
    def __init__(self, block, layers, baseWidth=26, scale=4, num_classes=1):
        ...
        self.center = DecoderBlock(
            in_channels=filters[3] * 4,
            n_filters=filters[3],
            kernel_size=decoder_kernel_size,
            is_deconv=is_deconv,
        )
        self.decoder4 = DecoderBlock(
            in_channels=filters[3] + filters[2] * 4,
            n_filters=filters[2],
            kernel_size=decoder_kernel_size,
            is_deconv=is_deconv,
        )
        self.decoder3 = DecoderBlock(
            in_channels=filters[2] + filters[1] * 4,
            n_filters=filters[1],
            kernel_size=decoder_kernel_size,
            is_deconv=is_deconv,
        )
        self.decoder2 = DecoderBlock(
            in_channels=filters[1] + filters[0] * 4,
            n_filters=filters[0],
            kernel_size=decoder_kernel_size,
            is_deconv=is_deconv,
        )
        self.decoder1 = DecoderBlock(
            in_channels=filters[0] + filters[0],
            n_filters=filters[0],
            kernel_size=decoder_kernel_size,
            is_deconv=is_deconv,
        )
...
if __name__ == "__main__":
    ...
    # plt.imshow(model_image.detach().numpy()[0][0])
    # plt.show()
